...
uint32_t amd_am79c973::HandleInterrupt(uint32_t esp)
{
    ...
    {
        printf("AMD_COLLISION ERROR\n");
    }
    if((temp & 0x1000) == 0x1000)
    {
        printf("AMD_MISSED FRAME\n");
    }
    if((temp & 0x0800) == 0x0800)
    {
        printf("AMD_MEMORY ERROR\n");
    }
...
void amd_am79c973::Receive()
{
    ...
    {
        // check Error Bit, && check Start-of-Packet Bit and End-of-Packet Bit
        if(!(recvBufferDescr[currentRecvBuffer].flags & 0x40000000) && (recvBufferDescr[currentRecvBuffer].flags & 0x03000000) == 0x03000000)
        {
            ...
            uint8_t* buffer = (uint8_t*)(recvBufferDescr[currentRecvBuffer].address); // copy address of the buffer

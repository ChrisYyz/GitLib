...
namespace myos 
{
    namespace net 
    {
        ...
        struct TransmissionControlProtocolHeader
        {
            ...
            common::uint32_t sequenceNumber; // 网络包乱序
            common::uint32_t acknowledgementNumber; // 确认

            common::uint8_t reserved : 4;
            common::uint8_t headerSize32 : 4; // 首部长，跟reserved结合
            common::uint8_t flags; // 3位

            common::uint16_t windowSize; // 窗口缓存，控制流速
            common::uint16_t checksum; // 检查是否损坏
        ...
        class TransmissionControlProtocolProvider : public net::InternetProtocolHandler
        {
        protected:
        ...
        public:
            TransmissionControlProtocolProvider(net::InternetProtocolProvider* backend);
